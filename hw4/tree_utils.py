...
class InternalDecisionNode(object):
    ...
    def predict(self, x_TF):
        ...
        for idx, sample in enumerate(x_TF):
            if sample[self.feat_id] < self.thresh_val:
                ids_x_TF_left.append(idx)
            else:
                ids_x_TF_right.append(idx)

        #print('ids_x_TF_left', ids_x_TF_left)
        #('ids_x_TF_right', ids_x_TF_right)

        samples_left = np.asarray([x_TF[i] for i in ids_x_TF_left])
        samples_right = np.asarray([x_TF[i] for i in ids_x_TF_right])

        #print('samples_left', samples_left.shape)
        #print('samples_right', samples_right.shape)

        # TODO ask the left child for its predictions (call 'predict')
        yhat_left = self.left_child.predict(samples_left)
        # TODO ask the right child for its predictions (call 'predict')
        yhat_right = self.right_child.predict(samples_right)

        #('yhat_left', yhat_left)
        #print('yhat_right', yhat_right)


        # TODO aggregate all predictions and return one array
        # Hint: Make sure to preserve the order of examples as in the input.
        yhat_T = np.zeros(T, dtype=np.float64) # TODO fixme
        idx_l = 0
        idx_r = 0
        for i in range(T):
            if i in ids_x_TF_left:
                yhat_T[i] = yhat_left[idx_l]
                idx_l = idx_l + 1
            else:
                yhat_T[i] = yhat_right[idx_r]
                idx_r = idx_r + 1
        return yhat_T
...
class LeafNode(object):
    ...
    def predict(self, x_TF):
        ...
        yhat_T = np.mean(self.y_N) * np.ones(T) #-1.2345 * np.ones(T) # TODO fixme
        return yhat_T
